...
// .wrangler/tmp/bundle-3mXBPI/checked-fetch.js
...
// .wrangler/tmp/pages-EB1yu1/functionsWorker-0.04701822822363133.mjs
...
__name(removeAccents2, "removeAccents2");
__name2(removeAccents2, "removeAccents");
var stopWords = /* @__PURE__ */ new Set(["de", "a", "o", "que", "e", "do", "da", "em", "um", "para", "\xE9", "com", "n\xE3o", "uma", "os", "no", "se", "na", "por", "mais", "as", "dos", "como", "mas", "foi", "ao", "ele", "das", "tem", "\xE0", "seu", "sua", "ou", "ser", "quando", "muito", "h\xE1", "nos", "j\xE1", "est\xE1", "eu", "tamb\xE9m", "s\xF3", "pelo", "pela", "at\xE9", "isso", "ela", "entre", "era", "depois", "sem", "mesmo", "aos", "ter", "seus", "quem", "nas", "me", "esse", "eles", "est\xE3o", "voc\xEA", "tinha", "foram", "essa", "num", "nem", "suas", "meu", "\xE0s", "minha", "t\xEAm", "numa", "pelos", "elas", "havia", "seja", "qual", "ser\xE1", "n\xF3s", "tenho", "lhe", "deles", "essas", "esses", "pelas", "este", "fosse", "dele", "tu", "te", "voc\xEAs", "vos", "lhes", "meus", "minhas", "teu", "tua", "teus", "tuas", "nosso", "nossa", "nossos", "nossas", "dela", "delas", "esta", "estes", "estas", "aquele", "aquela", "aqueles", "aquelas", "isto", "aquilo", "estou", "est\xE1", "estamos", "est\xE3o", "estive", "esteve", "estivemos", "estiveram", "estava", "est\xE1vamos", "estavam", "estivera", "estiv\xE9ramos", "esteja", "estejamos", "estejam", "estivesse", "estiv\xE9ssemos", "estivessem", "estiver", "estivermos", "estiverem", "hei", "h\xE1", "havemos", "h\xE3o", "houve", "houvemos", "houveram", "houvera", "houv\xE9ramos", "haja", "hajamos", "hajam", "houvesse", "houv\xE9ssemos", "houvessem", "houver", "houvermos", "houverem", "houverei", "houver\xE1", "houveremos", "houver\xE3o", "houveria", "houver\xEDamos", "houveriam", "sou", "somos", "s\xE3o", "era", "\xE9ramos", "eram", "fui", "foi", "fomos", "foram", "fora", "f\xF4ramos", "seja", "sejamos", "sejam", "fosse", "f\xF4ssemos", "fossem", "for", "formos", "forem", "serei", "ser\xE1", "seremos", "ser\xE3o", "seria", "ser\xEDamos", "seriam", "tenho", "tem", "temos", "t\xE9m", "tinha", "t\xEDnhamos", "tinham", "tive", "teve", "tivemos", "tiveram", "tivera", "tiv\xE9ramos", "tenha", "tenhamos", "tenham", "tivesse", "tiv\xE9ssemos", "tivessem", "tiver", "tivermos", "tiverem", "terei", "ter\xE1", "teremos", "ter\xE3o", "teria", "ter\xEDamos", "teriam", "me", "manda", "envia", "lista", "mostre", "fala", "diz", "ai", "alguma", "algum", "coisa", "sobre", "dos", "das", "ent\xE3o", "favor", "poderia", "gostaria", "saber", "se", "tipo", "exemplo", "exercicio", "exercicios", "prova", "vestibular", "ano", "materia"]);
function filtrarQuestoes(questoes, query) {
  if (!Array.isArray(questoes)) {
    console.warn("[WARN] filtrarQuestoes: 'questoes' n\xE3o \xE9 array.");
    return [];
  }
  if (typeof query !== "string" || query.trim() === "") {
    console.warn("[WARN] filtrarQuestoes: 'query' inv\xE1lida ou vazia.");
    return [];
  }
  const queryNormalized = removeAccents2(query.toLowerCase());
  if (!queryNormalized) {
    console.warn("[WARN] filtrarQuestoes: 'queryNormalized' vazia.");
    return [];
  }
  let palavrasChave = [];
  try {
    if (typeof queryNormalized === "string") {
      ...
      console.warn("[WARN] filtrarQuestoes: queryNormalized n\xE3o \xE9 string ap\xF3s removeAccents.");
      return [];
    }
  } catch (e) {
    console.error("[ERRO] filtrarQuestoes: Erro ao processar palavras-chave:", e);
    return [];
  }
  if (palavrasChave.length === 0) {
    console.log("[LOG] filtrarQuestoes: Nenhuma palavra-chave \xFAtil.");
    return [];
  }
  const resultadosComPontuacao = questoes.map((q) => {
    if (!q || typeof q !== "object") {
      console.warn("[WARN] filtrarQuestoes: Item inv\xE1lido.", q);
      return { questao: q, score: 0, match: false };
    }
    const ano = (q.ano ?? "").toString();
    const etapa = (q.etapa ?? "").toString();
    const materia = removeAccents2((q.materia || "").toLowerCase());
    const topico = removeAccents2((q.topico || "").toLowerCase());
    const textoQuestao = removeAccents2((q.texto_questao || "").toLowerCase());
    ...
    palavrasChave.forEach((palavra) => {
      try {
        ...
        console.error("[ERRO] filtrarQuestoes: Erro no 'includes':", e);
      }
    });
    return { questao: q, score, match: match2 };
  }).filter((item) => item.match).sort((a, b) => b.score - a.score);
  return resultadosComPontuacao.map((item) => item.questao);
}
__name(filtrarQuestoes, "filtrarQuestoes");
__name2(filtrarQuestoes, "filtrarQuestoes");
function parseAiGeneratedQuestion2(aiText) {
  console.log("[LOG] Tentando parsear texto da IA para quest\xE3o...");
  if (typeof aiText !== "string" || !aiText) return null;
  try {
    let enunciado = null;
    const alternativas = [];
    let respostaLetra = null;
    let materia = "Indefinida";
    let topico = "Indefinido";
    let textoProcessado = aiText;
    const referencia = "Quest\xE3o gerada por IA.";
    ...
    if (materiaMatch?.[1]) {
      ...
      textoProcessado = textoProcessado.substring(materiaMatch[0].length).trim();
      console.log("[LOG] Parse: Mat\xE9ria encontrada:", materia);
    } else {
      console.warn("[WARN] Parse: Mat\xE9ria n\xE3o encontrada no formato esperado.");
    }
    const topicoMatch = textoProcessado.match(topicoRegex);
    if (topicoMatch?.[1]) {
      topico = topicoMatch[1].trim();
      textoProcessado = textoProcessado.substring(topicoMatch[0].length).trim();
      console.log("[LOG] Parse: T\xF3pico encontrado:", topico);
    } else {
      console.warn("[WARN] Parse: T\xF3pico n\xE3o encontrado no formato esperado.");
    }
    ...
    if (enunciadoMatch?.[1]) {
      ...
      if (firstAltIndex > 0) {
        ...
        console.warn("[WARN] Parse: Enunciado n\xE3o extra\xEDdo.");
        return null;
      }
    }
    if (!enunciado) {
      console.warn("[WARN] Parse: Enunciado vazio.");
      return null;
    }
    let match2;
    while ((match2 = alternativaRegex.exec(textoProcessado)) !== null) {
      if (match2.index === alternativaRegex.lastIndex) {
        alternativaRegex.lastIndex++;
      }
      const letra = match2[1]?.toUpperCase();
      const texto = match2[2]?.trim();
      if (letra && texto) {
    ...
    if (respostaMatch?.[1]) {
      ...
      if (!alternativas.some((alt) => alt.letra === respostaLetra)) {
        console.warn(`[WARN] Parse: Resposta (${respostaLetra}) n\xE3o encontrada nas alternativas.`);
        respostaLetra = null;
      }
    }
    if (!respostaLetra) {
      console.warn("[WARN] Parse: Resposta correta n\xE3o encontrada.");
      return null;
    }
    const generatedQuestion = {
      id: `gen-${Date.now()}`,
      ano: null,
      // Não tem ano específico
      etapa: null,
      // Não tem etapa específica
      materia,
      // <<< USA A MATÉRIA EXTRAÍDA
      topico,
      // <<< USA O TÓPICO EXTRAÍDO
      texto_questao: enunciado,
      referencia,
      // Indica origem IA
      alternativas,
    ...
    console.log("[LOG] Parse: Quest\xE3o gerada parseada com sucesso.");
...
__name(parseAiGeneratedQuestion2, "parseAiGeneratedQuestion2");
__name2(parseAiGeneratedQuestion2, "parseAiGeneratedQuestion");
async function onRequestPost2(context) {
  ...
  try {
    ...
    const r2Bucket = env.QUESTOES_PAVE_BUCKET;
    const modelName = env.MODEL_NAME || "gemini-2.0-flash";
    if (!r2Bucket || !geminiApiKey) {
      throw new Error("Configura\xE7\xE3o interna incompleta.");
    }
    console.log(`[LOG] ${functionName}: Configs OK. Modelo: ${modelName}`);
    let requestData;
    try {
      ...
      return new Response(JSON.stringify({ error: "Requisi\xE7\xE3o inv\xE1lida." }), { status: 400 });
    }
    const history = requestData?.history;
    if (!Array.isArray(history) || history.length === 0) {
      return new Response(JSON.stringify({ error: "Hist\xF3rico inv\xE1lido." }), { status: 400 });
    }
    const lastUserMessage = history.findLast((m) => m.role === "user");
    const userQuery = typeof lastUserMessage?.parts?.[0]?.text === "string" ? lastUserMessage.parts[0].text.trim() : null;
    if (!userQuery) {
      return new Response(JSON.stringify({ error: "Query inv\xE1lida no hist\xF3rico." }), { status: 400 });
    }
    console.log(`[LOG] ${functionName}: \xDAltima query: "${userQuery}"`);
    ...
    try {
      ...
      if (r2Object !== null) {
        ...
        if (Array.isArray(allQuestionsData)) {
          ...
          console.warn(`[WARN] ${functionName}: Conte\xFAdo R2 n\xE3o \xE9 array.`);
          allQuestionsData = [];
        }
      } else {
        console.warn(`[WARN] ${functionName}: questoes.json n\xE3o encontrado no R2.`);
      }
    } catch (e) {
      console.error(`[ERRO] ${functionName}: Falha ao ler/processar R2:`, e);
    }
    console.log(`[LOG] ${functionName}: ${questoesRelevantes.length} quest\xF5es relevantes filtradas do R2.`);
    const queryLower = userQuery.toLowerCase();
    const requestKeywords = ["quest\xE3o", "quest\xF5es", "exerc\xEDcio", "exerc\xEDcios", "exemplo", "mostre", "mande", "liste", "quero ver"];
    const createKeywords = ["crie", "invente", "elabore", "gere uma quest\xE3o", "fa\xE7a uma quest\xE3o", "crie uma quest\xE3o", "gere"];
    ...
    if (isAskingForExisting && !isAskingToCreate && questoesRelevantes.length > 0) {
      console.log(`[LOG] ${functionName}: Inten\xE7\xE3o: Mostrar quest\xE3o existente do R2.`);
      const qToShow = questoesRelevantes[0];
      const topico = qToShow?.topico || qToShow?.materia || "relacionado";
      commentary = `Ok! Encontrei uma quest\xE3o sobre ${topico} para voc\xEA praticar:`;
      questionsToReturn = [{
        ano: qToShow?.ano,
        etapa: qToShow?.etapa,
        materia: qToShow?.materia,
        topico: qToShow?.topico,
        texto_questao: qToShow?.texto_questao,
        referencia: qToShow?.referencia,
        alternativas: qToShow?.alternativas,
        resposta_letra: qToShow?.resposta_letra
      }];
    } else if (isAskingToCreate) {
      console.log(`[LOG] ${functionName}: Inten\xE7\xE3o: Criar quest\xE3o com IA.`);
      const creationPrompt = `Crie uma quest\xE3o de m\xFAltipla escolha (A, B, C, D, E) IN\xC9DITA no estilo do PAVE UFPEL sobre o seguinte t\xF3pico ou instru\xE7\xE3o: "${userQuery}".
        Formate sua resposta **exatamente** com as seguintes se\xE7\xF5es, cada uma em uma nova linha:
        Mat\xE9ria: [Nome da Mat\xE9ria Principal, ex: F\xEDsica, Hist\xF3ria, Biologia]
        T\xF3pico: [Nome do T\xF3pico Espec\xEDfico dentro da mat\xE9ria, ex: Cinem\xE1tica, Iluminismo, Gen\xE9tica]
        Enunciado: [Texto completo do enunciado aqui, pode ter m\xFAltiplas linhas]
        A) [Texto da alternativa A]
        B) [Texto da alternativa B]
        C) [Texto da alternativa C]
        D) [Texto da alternativa D]
        E) [Texto da alternativa E]
        Resposta Correta: [Apenas a LETRA mai\xFAscula correta, ex: C]`;
      console.log(`[LOG] ${functionName}: Enviando prompt de CRIA\xC7\xC3O para Gemini.`);
      try {
        ...
        if (!aiGeneratedText && response.promptFeedback?.blockReason) {
          commentary = `(N\xE3o posso criar essa quest\xE3o: ${response.promptFeedback.blockReason})`;
        } else if (aiGeneratedText) {
          const parsedQuestion = parseAiGeneratedQuestion2(aiGeneratedText);
          if (parsedQuestion) {
            commentary = "Certo, elaborei esta quest\xE3o para voc\xEA:";
            questionsToReturn = [parsedQuestion];
          } else {
            console.warn(`[WARN] ${functionName}: Falha ao parsear quest\xE3o gerada pela IA.`);
            commentary = `Tentei criar uma quest\xE3o, mas n\xE3o consegui format\xE1-la corretamente. Aqui est\xE1 o texto gerado:

${aiGeneratedText}`;
            questionsToReturn = [];
          }
        } else {
          commentary = "(A IA n\xE3o gerou a quest\xE3o.)";
        }
      } catch (error) {
        console.error(`[ERRO] ${functionName}: Erro Gemini CRIA\xC7\xC3O:`, error);
        commentary = `(Desculpe, erro ao criar a quest\xE3o: ${error.message})`;
      }
    } else {
      console.log(`[LOG] ${functionName}: Inten\xE7\xE3o: Conversa ou quest\xE3o n\xE3o encontrada.`);
      let conversationContext = "Nenhuma quest\xE3o relevante encontrada na base para esta pergunta.";
      if (isAskingForExisting && questoesRelevantes.length === 0) {
        conversationContext += " (O usu\xE1rio pediu quest\xF5es existentes, mas a busca inicial n\xE3o retornou resultados.)";
        ...
        conversationContext = `Foram encontradas ${questoesRelevantes.length} quest\xF5es sobre "${topicosEncontrados}", mas o usu\xE1rio n\xE3o pediu para v\xEA-las explicitamente.`;
      }
      const conversationPrompt = `Voc\xEA \xE9 um assistente PAVE UFPEL amig\xE1vel. Responda \xE0 \xDALTIMA pergunta do usu\xE1rio ("${userQuery}") de forma conversacional, usando o hist\xF3rico para contexto. Contexto adicional: ${conversationContext}. Se souber que o usu\xE1rio pediu quest\xF5es mas n\xE3o foram encontradas, explique isso. N\xE3o invente quest\xF5es. Seja conciso.`;
...
[Instru\xE7\xE3o para IA]: ${conversationPrompt}` });
        } else {
        ...
        if (!commentary && response.promptFeedback?.blockReason) {
          commentary = `(Bloqueado: ${response.promptFeedback.blockReason})`;
        } else if (!commentary) {
          commentary = "(N\xE3o consegui gerar uma resposta.)";
        }
...
async function onRequest3(context) {
  ...
  return new Response(`M\xE9todo ${context.request.method} n\xE3o permitido. Use POST.`, { status: 405 });
}
...
// .wrangler/tmp/bundle-3mXBPI/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-3mXBPI/middleware-loader.entry.ts
...
@google/generative-ai/dist/index.mjs:
   ...
   *)
*/
//# sourceMappingURL=functionsWorker-0.04701822822363133.js.map
